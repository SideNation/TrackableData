...
namespace TrackableData
{
    public static class TrackableResolver
    {
        public static bool IsTrackablePoco(Type type) =>
            typeof(ITrackablePoco).IsAssignableFrom(type);

        public static bool IsTrackableContainer(Type type) =>
            typeof(ITrackableContainer).IsAssignableFrom(type);

        public static bool IsTrackableDictionary(Type type) =>
            type.IsGenericType && type.GetGenericTypeDefinition() == typeof(TrackableDictionary<,>);

        public static bool IsTrackableSet(Type type) =>
            type.IsGenericType && type.GetGenericTypeDefinition() == typeof(TrackableSet<>);

        public static bool IsTrackableList(Type type) =>
            type.IsGenericType && type.GetGenericTypeDefinition() == typeof(TrackableList<>);
