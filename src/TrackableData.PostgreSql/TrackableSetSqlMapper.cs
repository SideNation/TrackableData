...
namespace TrackableData.PostgreSql
{
    public class TrackableSetSqlMapper<T>
    {
        ...
        public async Task<int> ResetTableAsync(DbConnection connection)
        {
            ...
            _logger.LogDebug("TrackableSetSqlMapper<{Type}>.ResetTableAsync:\n{Sql}", typeof(T).Name, sql);
                                           ...
                                           params object[] keyValues)
        {
            ...
            if (string.IsNullOrEmpty(sql))
                return 0;
            _logger.LogDebug("TrackableSetSqlMapper<{Type}>.CreateAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
            }
        ...
        public async Task<int> DeleteAsync(DbConnection connection, params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackableSetSqlMapper<{Type}>.DeleteAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
            }
                                               ...
                                               params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackableSetSqlMapper<{Type}>.LoadAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                                         ...
                                         params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackableSetSqlMapper<{Type}>.SaveAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
            }
