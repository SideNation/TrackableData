...
namespace TrackableData.PostgreSql
{
    public class TrackableDictionarySqlMapper<TKey, TValue>
    {
        ...
        public async Task<int> ResetTableAsync(DbConnection connection)
        {
            ...
            _logger.LogDebug("TrackableDictionarySqlMapper<{TKey},{TValue}>.ResetTableAsync:\n{Sql}",
                typeof(TKey).Name, typeof(TValue).Name, sql);
                                           ...
                                           params object[] keyValues)
        {
            ...
            if (string.IsNullOrEmpty(sql))
                return 0;
            _logger.LogDebug("TrackableDictionarySqlMapper<{TKey},{TValue}>.CreateAsync:\n{Sql}",
                typeof(TKey).Name, typeof(TValue).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
        ...
        public async Task<int> DeleteAsync(DbConnection connection, params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackableDictionarySqlMapper<{TKey},{TValue}>.DeleteAsync:\n{Sql}",
                typeof(TKey).Name, typeof(TValue).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
                                                                 ...
                                                                 params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackableDictionarySqlMapper<{TKey},{TValue}>.LoadAsync:\n{Sql}",
                typeof(TKey).Name, typeof(TValue).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                                         ...
                                         params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackableDictionarySqlMapper<{TKey},{TValue}>.SaveAsync:\n{Sql}",
                typeof(TKey).Name, typeof(TValue).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
