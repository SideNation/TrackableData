...
namespace TrackableData.PostgreSql
{
        ...
        where T : ITrackablePoco<T>
    {
                                      ...
                                      ITrackableLogger logger)
        {
            ...
            if (primaryKeyColumns.Any() == false)
                throw new ArgumentException("At least one primary key should be provided.");
        ...
        public async Task<int> ResetTableAsync(DbConnection connection)
        {
            ...
            _logger.LogDebug("TrackablePocoSqlMapper<{Type}>.ResetTableAsync:\n{Sql}", typeof(T).Name, sql);
        ...
        public async Task<int> CreateAsync(DbConnection connection, T value, params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackablePocoSqlMapper<{Type}>.CreateAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
        ...
        public async Task<int> DeleteAsync(DbConnection connection, params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackablePocoSqlMapper<{Type}>.DeleteAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
            }
        ...
        public Task<T> LoadAsync(DbConnection connection, params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackablePocoSqlMapper<{Type}>.LoadAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
        ...
        {
            var sql = BuildSqlForLoad(keyValues);
            _logger.LogDebug("TrackablePocoSqlMapper<{Type}>.LoadAllAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                using (var reader = command.ExecuteReader())
                {
                                         ...
                                         params object[] keyValues)
        {
            ...
            _logger.LogDebug("TrackablePocoSqlMapper<{Type}>.SaveAsync:\n{Sql}", typeof(T).Name, sql);
            using (var command = _sqlProvider.CreateDbCommand(sql, connection))
            {
                return await command.ExecuteNonQueryAsync();
            }
